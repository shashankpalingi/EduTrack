...
interface AuthContextType {
  ...
  loginWithGithub: (role?: string) => Promise<UserCredential>;
  logout: () => Promise<void>;
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const register = async (email: string, password: string, displayName: string, role: 'teacher' | 'student' = 'student') => {
    try {
      clearError();
      const result = await createUserWithEmailAndPassword(auth, email, password);
      const user = result.user;

      // Create a user profile using the userService
  ...
  const loginWithGoogle = async (role: string = 'student') => {
    try {
      ...
      if (user) {
        await createUserProfileIfNeeded(user, role);
      }
  ...
  const loginWithGithub = async (role: string = 'student') => {
    try {
      clearError();
  ...
  const createUserProfileIfNeeded = async (user: User, role: string = 'student') => {
    try {
